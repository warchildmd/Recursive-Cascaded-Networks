...
def main():
    ...
    with tf.Session(config=tf.ConfigProto()) as sess:
        ...
        while True:
            ...
            # print(fd.keys())
            fd['voxel1'] = np.array([np.expand_dims(gaussian_filter(x.squeeze(), sigma), -1) for x in fd['voxel1']])
            fd['voxel2'] = np.array([np.expand_dims(gaussian_filter(x.squeeze(), sigma), -1) for x in fd['voxel2']])
            # print(fd['voxel1'].shape)
            # print(fd['voxel2'].shape)
            # break
